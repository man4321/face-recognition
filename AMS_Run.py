    ...
    def fill_attendance():
        ...
        if subb=='':
            ...
            def create_csv():
                ...
                csv_name=r'D:\Final\Attendace_management_system-master\Attendance\Manually Attendance'+DB_table_name+'.csv';
                with open(csv_name, "w") as csv_file:
                    csv_writer = csv.writer(csv_file);
                    
                    csv_writer.writerow(['Enrolment','Name','Date','Time','Total Time'])  # write headers
            ...
            def attf():
                import subprocess
                subprocess.Popen("D:\Final\Attendace_management_system-master\Attendance\Manually Attendance")
...
def subjectchoose():
    def Fillattendances():
        ...
        if time.time() < future:
            if sub == '':
                ...
                df = pd.read_csv("D:\Final\Attendace_management_system-master\StudentDetails/StudentDetails.csv")
                cam = cv2.VideoCapture(0)
                font = cv2.FONT_HERSHEY_SIMPLEX
                col_names = ['Enrollment', 'Name', 'Date', 'Time',"TotalTime"]
                ...
                while True:
                    ...
                    for (x, y, w, h) in faces:
                        ...
                        if (conf <70):
                            ...
                            mins =60;
                            secs =0;
                            TotalTime =(abs(int(mins)-int(min))+ (int(sec)-secs)*0)
                                        
                            aa = df.loc[df['Enrollment'] == Id]['Name'].values
                            global tt
                            tt = str(Id) + "-" + aa
                            En = '15624031' + str(Id)
                            attendance.loc[len(attendance)] = [Id, aa, date, timeStamp,TotalTime]
                ...
                insert_data =  "INSERT INTO " + DB_Table_name + " (ID,ENROLLMENT,NAME,DATE,TIME,TotalTime) VALUES (0, %s, %s, %s,%s,%s)"
                VALUES = (str(Id), str(aa), str(date), str(timeStamp),str("00:40:00"));
                try:
                    cursor.execute(sql)  ##for create a table
                ...
                root.configure(background='snow')
                cs = 'D:\Final\Attendace_management_system-master\Attendance' + fileName
    ...
    def Attf():
        import subprocess
        subprocess.Popen(r'explorer /select,"D:\Final\Attendace_management_system-master\Attendance')
    ...
    def log_in():
        ...
        if username == 'Manohar' :
            if password == '123':
                ...
                cs = 'D:\Final\Attendace_management_system-master\StudentDetails\StudentDetails.csv';
                with open(cs, newline="") as file:
                    reader = csv.reader(file)
                    r = 0
